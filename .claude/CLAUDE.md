...
- Implementation = code + tests + documentation (domain help codeunit, HTML help, README)
...
- `Storage Takeover ori` (10035676) takes data over from the published
  *Origo Cloud Events Storage* app (`7acf9361-f558-442b-a516-f5e5dd92aecb`) on first install per
  ...
  It is **generated by `tools/gen_install.py`** - regenerate, never hand-edit.
...
- Full message-type test reports live in
  `app/docs/Bifrost_Hnitbjorg_MessageType_TestReport_<date>.md`, generated by
  `tools/make_report.py` from JSON-line scenario logs (see `tools/hnitbjorg_types.txt` for the
  type list) - the open defect list from the last run is in that report's "Defects and
  observations" section - fix from there.
